...
$(document).on('click','#verMasFactura', function(){
		...
		.done(function(respuesta){
			...
			document.getElementById('nPaquete').value=factura[5];
			document.getElementById('lPaquete').value=factura[6];
			document.getElementById('aPaquete').value=factura[7];
			document.getElementById('anPaquete').value=factura[8];
			document.getElementById('pPaquete').value=factura[9];
		})
		.fail(function () {
			console.log('error');
		})	
	...
	$("#origen,#destino").keyup(function(){
		...
		.done(function(respuesta){
			...
			if(respuesta!=""){
				...
				//inabilitar para no poder ser elegido
				$("#dropDownRuta").append("<option>No hay elemetos para ésta ruta!!!</option>");
			}
			//$("#modalAgregarViaje").modal("show");
			//alert(respuesta);
		})
...
$(document).on('click','#eliminarArt', function(){

	if (confirm('Está seguro que desea eliminar este registro?')) {
		var datos=$(this).data('cve_conpaq');
		var num_fact=$(this).data('num_fact');
		var arreglo = datos.split('@');
		claveP = arreglo[0];
		cantidad = arreglo[1];
		precio = arreglo[2];
		paquete = document.getElementById('nPaquete').value;
		$.ajax({
			url:'eliminarArti.php',
			type:'POST',
			dataType:'html',
			data:{claveP:claveP, cantidad:cantidad, precio:precio, paquete:paquete},
			success: function(data){
...
$(document).on('click','#cambiarStatusP', function(){
	if (confirm('¿El paqute se encuentra correctamente?')) {
		var claveF=$(this).data('num_fact');
		$.ajax({
			url:'cambStaF.php',
			type:'POST',
			dataType:'html',
...
$(document).on('click','#updatePaquete', function(){
	if (confirm('¿Los datos ya se encuentran correctos?')) {
		var claveP=$("#nPaquete").val();
		var largo=$("#lPaquete").val();
		var alto=$("#aPaquete").val();
		var ancho=$("#anPaquete").val();
		var peso =$("#pPaquete").val();

		$.ajax({
			url:'actualizarPaq.php',
			type:'POST',
			dataType:'html',
			data:{claveP:claveP, largo:largo, alto:alto, ancho:ancho, peso:peso},
			success: function(data){
				location.reload(true);
			}
		})
